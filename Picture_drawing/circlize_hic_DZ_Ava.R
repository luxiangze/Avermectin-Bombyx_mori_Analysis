...
print("Adding DZ_loop link tracks")

# 添加loop数据
DZ_loop <- read.table("/home/gyk/project/lw_hic_snHiC/09_Loop_detection_HiCexplorer/DZ/DZ_mapQ15_20kb_loops.bedpe", header = FALSE, sep = "\t")
colnames(DZ_loop) <- c("chr1", "start1", "end1", "chr2", "start2", "end2", "pvalue")
DZ_loops1 <- DZ_loop[, c("chr1", "start1", "end1", "pvalue")]
colnames(DZ_loops1) <- c("chr", "start", "end", "pvalue")
DZ_loops2 <- DZ_loop[, c("chr2", "start2", "end2", "pvalue")]
colnames(DZ_loops2) <- c("chr", "start", "end", "pvalue")
# 合并两个数据框
DZ_loops <- rbind(DZ_loops1, DZ_loops2)

# 统计loop pvalue数据的值
DZ_loops$pvalue <- -log10(DZ_loops$pvalue)
summary(DZ_loops$pvalue)
p_max <- max(DZ_loops$pvalue)

circos.genomicTrack(DZ_loops,
  track.height = 0.05,
  bg.border = NA,
  bg.col = "#FFFFFF",
...
print("Adding Ava_loop link2 tracks")

# 添加loop数据
Ava_loop <- read.table("/home/gyk/project/lw_hic_snHiC/09_Loop_detection_HiCexplorer/Ava/Ava_mapQ15_20kb_loops.bedpe", header = FALSE, sep = "\t")
colnames(Ava_loop) <- c("chr1", "start1", "end1", "chr2", "start2", "end2", "pvalue")
Ava_loops1 <- Ava_loop[, c("chr1", "start1", "end1", "pvalue")]
colnames(Ava_loops1) <- c("chr", "start", "end", "pvalue")
Ava_loops2 <- Ava_loop[, c("chr2", "start2", "end2", "pvalue")]
colnames(Ava_loops2) <- c("chr", "start", "end", "pvalue")
# 合并两个数据框
Ava_loops <- rbind(Ava_loops1, Ava_loops2)

# 统计loop pvalue数据的值
Ava_loops$pvalue <- -log10(Ava_loops$pvalue)
summary(Ava_loops$pvalue)
p_max <- max(Ava_loops$pvalue)

circos.genomicTrack(Ava_loops,
  track.height = 0.05,
  bg.border = NA,
  bg.col = "#FFFFFF",
  ylim = c(0, p_max), # 使用实际的数据范围
...
print("Adding DZ_compartment tracks")
